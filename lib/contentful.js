...
export async function fetchTriageQuestions() {
  ...
  try {
    // Define the parameters in an object
    const paramsForContentful = {
      content_type: 'assessmentQuestion',
      'fields.questionModule': 'Triage',
      order: 'fields.displayOrder',
    };

    // Log the parameters object right before you use it
    console.log("[contentful.js] Parameters being sent to Contentful client.getEntries():", JSON.stringify(paramsForContentful));

    const entries = await client.getEntries(paramsForContentful); // Use the object here
